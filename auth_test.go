...
var (
	...
	testPaths      = []string{"/DemoPass/index.html", "/DemoPass/", "/nonexistent", "/nonexistentdir/", "/TestingPass/"}
	testErrors     = []string{"err", "err", "forbid"}
)
...
func Test_DetectPasswd(t *testing.T) {
	...
	for i := 0; i < len(testPaths); i++ {
		if i > 1 {
			if !reflect.DeepEqual(DetectPasswd(testPaths[i], "html"), []string{testErrors[i-2]}) {
				t.Fatal("DetectPasswd is not functioning correctly!")
			}
			continue
		}
		if !reflect.DeepEqual(DetectPasswd(testPaths[i], "html"), expectedLogins) {
			t.Fatal("DetectPasswd is not functioning correctly!")
		}
	}
}
